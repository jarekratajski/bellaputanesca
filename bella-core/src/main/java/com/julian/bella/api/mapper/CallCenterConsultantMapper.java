package com.julian.bella.api.mapper;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.julian.bella.api.dto.CallCenterConsultantDto;
import com.julian.bella.api.dto.UserDto;
import com.julian.bella.domain.CallCenterConsultant;
import com.julian.bella.domain.User;

@Component
public class CallCenterConsultantMapper implements GenericMapper<CallCenterConsultant, CallCenterConsultantDto> {

	UserMapper userMapper;

	@Autowired
	public CallCenterConsultantMapper(UserMapper userMapper) {
		this.userMapper = userMapper;
	}

	@Override
	public CallCenterConsultantDto sourceToDto(CallCenterConsultant source) {
		if (source == null) {
			return null;
		}
		UserDto userDto = userMapper.sourceToDto(source.getUser());

		CallCenterConsultantDto dto = new CallCenterConsultantDto();
		
		dto.setActive(source.isActive())
			.setPesel(source.peselEncrypted)
			.setFirstName(source.getFirstName())
			.setLastName(source.getLastName())
			.setUserDto(userDto);
		return dto;
	}
	
	@Override
	public CallCenterConsultant dtoToNewSource(CallCenterConsultantDto dto) {
		if(dto == null) {
			return null;
		}
		User user = userMapper.dtoToNewSource(dto.getUserDto());

		CallCenterConsultant source = new CallCenterConsultant(dto.getPesel()); //sourceClass.getDeclaredConstructor(String.class).newInstance(dto.getPesel());
		
		source.setActive(dto.isActive());
		source.setFirstName(dto.getFirstName());
		source.setLastName(dto.getLastName());
		source.setUser(user);
		return source;
	}
	
	@Override
	public CallCenterConsultant dtoToUpdatedSource(CallCenterConsultant source, CallCenterConsultantDto dto) {
		if(dto == null) {
			return null;
		}
		if(source == null) {
			return dtoToNewSource(dto);
		}
		User user = userMapper.dtoToUpdatedSource(source.getUser(), dto.getUserDto());
		
		source.setActive(dto.isActive());
		source.setFirstName(dto.getFirstName());
		source.setLastName(dto.getLastName());
		source.setUser(user);
		return source;
	}
}